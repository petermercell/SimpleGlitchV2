...
{
  return fract(std::sin(static_cast<float>(y) * 12.9898f + noise_seed) * 43758.5453f);
}

// block index
inline int blockIndex(int current, int final_pos, int size)
{
...
{
  _bbox = static_cast<int>(noise_intensity);
...
void SimpleGlitchIop::engine(int y, int x, int r, ChannelMask channels, Row &out)
{
  ...
  int block_index = blockIndex(y, im_y, static_cast<int>(noise_height));
  // Set the Noise
  ...
  foreach(z, channels) {
    ...
    if(lineNoise < noise_freq) {
      int offset = static_cast<int>((lineNoise - noise_offset) * noise_intensity *
                                    NOISE_MULT);  // block offset
      ...
      std::copy(in[z] + x, in[z] + r,
                out.writable(z) +
                    x);  // Just copy the same pixel values to the current row (No pixel changes)
    }
  }
}
...
{
  return new NukeWrapper(new SimpleGlitchIop(node));
}
